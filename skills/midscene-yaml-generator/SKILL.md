...
5. **仅使用 schema 中已定义的关键字** — 优先参考本文档中的映射表和选中的模板文件，对合法关键字有疑问时再用 Read 工具读取 `schema/native-keywords.json` 确认
...
**常见致命错误速查**（参见下方 Anti-patterns 完整示例）：
- `aiInput` 后必须跟 `value:` 兄弟键 — 缺失 = 空操作
- Extended 模式必须同时有 `engine: extended` + `features: [...]`
- `while` 循环必须设置 `maxIterations` — 缺失 = 可能无限循环
...
> **何时用 `ai:`**: 仅当步骤不确定、需要 AI 自主规划路径时使用（如 `ai: "完成整个结账流程"`）。步骤明确时优先用 `aiTap`/`aiInput` — 更快更稳定。
...
# RIGHT — try/catch/finally 是同一步骤的兄弟键
- try:
    flow:
      - aiTap: "可能失败的操作"
  catch:             # ✓ 与 try 对齐，同级兄弟键
    flow:
      - ai: "错误恢复操作"
  finally:
    flow:
...
> **缩进规则**: `catch`/`finally` 与 `try` 是同一步骤对象的兄弟键，缩进对齐。
...
生成后验证：每个 `aiInput` 有 `value`；关键操作后有 `aiWaitFor`；Extended 模式有 `engine: extended` + `features`（参见关键规则 #1）；循环有 `maxIterations`（参见关键规则 #3）；敏感信息用 `${ENV:XXX}`；AI 指令包含位置/文字/颜色等特征。
...
2. **接收 Runner 错误上下文**：当 Runner 发送 `[ESCALATE]` 格式的错误时，解析错误类型、失败步骤和建议，针对性地重新生成或修改 YAML。响应时包含 `[FIX_FOR] <file>.yaml` 确认令牌
