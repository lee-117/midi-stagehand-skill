...
> **阅读导航**: 本手册分 5 个渐进级别。**L1-L3** 覆盖 Native 模式（页面操作、数据提取、断言），**L4-L5** 覆盖 Extended 模式（逻辑控制、系统集成）。后续章节为性能调优、多平台支持、CI/CD 集成和附录。根据需求跳转到对应级别即可。

---

## 快速概览

Midscene YAML 生态包含两种运行模式：

| | **Native 模式** | **Extended 超集模式** |
|---|---|---|
| **适用场景** | 页面操作、数据提取、简单断言 | 条件分支、循环、API 调用、并行任务 |
| **执行方式** | Midscene 引擎直接执行 | 先转译为 TypeScript，再执行 |
| **声明方式** | 默认模式，可省略 | 需声明 `engine: extended` |
...
# 使用 Puppeteer 模板（默认模板已改为 Playwright）
node scripts/midscene-run.js example.yaml --template puppeteer
...
交互动作：`aiTap`、`aiHover`、`aiInput`、`aiKeyboardPress`、`aiScroll`、`aiDoubleClick`、`aiRightClick`、`aiDragAndDrop`、`aiClearInput`、`aiLongPress`、`ai`（推荐别名 `aiAct`，`aiAction` 亦为有效别名）。
...
> **别名**: `aiAct` 是 `ai` 的完全等价别名（推荐使用），`aiAction` 亦为有效别名。三者可互换使用。
...
> **模板变量**: `sleep` 的值支持模板变量字符串，例如 `sleep: "${delayMs}"`，在 Extended 模式下可通过变量动态控制等待时间。
...
> **null 值**: `variables` 支持 `null` 类型值，例如 `initialData: null`。可用于声明一个初始为空的变量，后续通过 `aiQuery` 等动作赋值。
...
在循环中逐次收集数据并追加到数组，是常见的数据采集模式。使用 `data_transform` 的 `reduce` 或链式操作将每次迭代的结果累积到一个变量中。

```yaml
# 循环数据累积 — forEach 逐次追加到数组
engine: extended
      ...
      - loop:
          ...
          flow:
            ...
            - data_transform:
                source: "${allResults}"
                operation: reduce
                reducer: "acc.concat(${pageResults})"
                initial: []
                name: "allResults"

      # 循环结束后，allResults 包含所有关键词的搜索结果
      - aiAssert: "已完成所有关键词的搜索"

    output:
      filePath: "./output/all-search-results.json"
      dataName: "allResults"
```

> **关键点**: 每次迭代用 `data_transform` 将 `pageResults` 追加到 `allResults`，循环结束后 `allResults` 包含所有迭代的累积数据。

### 分页采集+数据合并

使用 `while` 循环翻页采集，结合数据累积模式合并所有页面的数据。

```yaml
# 分页采集+数据合并 — while 循环翻页，逐页追加
engine: extended

variables:
  siteUrl: "https://product-list.example.com/category/electronics"
  allItems: []
...
tasks:
      ...
      - loop:
          ...
          flow:
            ...
            - data_transform:
                source: "${allItems}"
                operation: reduce
                reducer: "acc.concat(${currentPage})"
                initial: []
                name: "allItems"

            # 判断是否有下一页
            - logic:
                if: "页面底部有'下一页'按钮且未被禁用（非灰色）"
                then:
                  - aiTap: "下一页"
                  - aiWaitFor: "新的商品列表页面加载完成"
                else:
                  - ai: "已到达最后一页，停止翻页"
  ...
  - name: 数据后处理
    ...
    output:
      filePath: "./output/all-products.json"
      dataName: "sortedItems"
```

> **运行方式**: `node scripts/midscene-run.js pagination-demo.yaml`。循环在"下一页"按钮不可用或达到 `maxIterations` 时终止。

### `aiActContext` — AI 操作上下文

`aiActContext` 是 `agent` 配置中的一个字段，用于向 AI 提供额外的业务背景知识。当页面内容存在歧义、专业术语或非标准 UI 时，AI 可能无法准确理解操作意图，此时 `aiActContext` 能显著提升识别准确度。

**工作原理**: `aiActContext` 的文本会作为上下文附加到每次 AI 调用中，帮助 AI 理解页面元素的含义和操作意图。

**适用场景**:
- 中英文混排页面，AI 需要知道哪种语言优先
- 包含医学、金融、法律等专业术语的页面
- 使用自定义组件库、非标准 UI 控件的页面
- 多步骤业务流程中需要理解业务背景的操作
...
```yaml
web:
  url: "https://intl-shop.example.com"

agent:
  aiActContext: >
    这是一个中英文混排的国际电商平台。
    导航栏和按钮使用英文（如 "Add to Cart", "Checkout"），
    商品描述和用户评价使用中文。
    "购物车"和"Cart"指同一功能。
    价格单位为人民币（¥）。
...
tasks:
  - name: 购买商品
    flow:
      - aiTap: "Add to Cart 按钮"
      - aiTap: "Checkout"
      - aiAssert: "结算页面显示了商品信息"
```
...
```yaml
web:
  url: "https://medical-portal.example.com"

agent:
  aiActContext: >
    这是一个医疗信息管理系统。页面使用以下专业术语：
    - "HbA1c" = 糖化血红蛋白检测指标
    - "BMI" = 体质指数
...
agent:
  aiActContext: >
    本系统使用自定义 UI 组件库，注意以下特殊交互：
    - 下拉选择器外观为灰色圆角矩形，点击后弹出选项列表
    - "时间范围选择器"是两个日期输入框，左边是开始日期右边是结束日期
    - 图表区域可以鼠标悬停显示详情 tooltip
    - 侧边栏的折叠按钮是一个小三角形图标
    - "状态标签"是圆形色块：绿色=正常，黄色=警告，红色=异常
...
AI 缓存可以显著减少重复执行时的 API 调用和运行时间。不同场景需要不同的缓存策略。

#### 何时启用缓存

| 场景 | 推荐策略 | 原因 |
|------|---------|------|
| 本地开发迭代 | `read-write` | 反复调试同一 YAML，避免重复 AI 调用 |
| 页面结构稳定的回归测试 | `read-write` | 相同页面结构命中缓存率高 |
| 生产环境使用预热缓存 | `read-only` | 只使用已验证的缓存，不写入新结果 |
| 预热缓存（首次运行） | `write-only` | 强制执行 AI 并写入缓存，不读取旧缓存 |

#### 何时禁用缓存

| 场景 | 推荐策略 | 原因 |
|------|---------|------|
| CI/CD 流水线 | `false` 或不设置 | 动态环境下缓存可能导致误判 |
| 页面内容频繁变化 | `false` | 缓存键基于页面上下文，内容变化缓存自动失效 |
| 首次验证正确性 | `false` | 需要确保 AI 结果准确，不依赖缓存 |
...
agent:
  cache: false
  testId: "ci-regression-${ENV:BUILD_NUMBER}"
```
...
理解变量的作用域有助于避免"变量未定义"的常见问题。

#### 顶层 `variables` — 全局作用域

在文件顶部 `variables` 块中声明的变量，对所有 task 可见，是跨 task 共享数据的标准方式。

```yaml
variables:
  username: "admin"       # 所有 task 都可以通过 ${username} 访问
  collectedData: []       # 跨 task 累积数据

tasks:
  - name: 任务一
    flow:
      - aiInput: "用户名"
        value: "${username}"    # 可访问顶层变量

  - name: 任务二
    flow:
      - aiAssert: "${username} 已登录"   # 同样可访问
```

#### 流程内变量 — task 内作用域

通过 `aiQuery`/`aiBoolean`/`aiNumber`/`aiString` 的 `name` 字段创建的变量，在当前 task 及后续 task 中可用（运行时赋值，按执行顺序累积）。
...
tasks:
  - name: 提取数据
    flow:
      - aiQuery:
          query: "用户名"
          name: "currentUser"    # 运行时创建 ${currentUser}

  - name: 使用数据
    flow:
      - aiAssert: "${currentUser} 显示在页面上"   # 可访问前面 task 创建的变量
```

#### `data_transform` 输出 — 与 `name` 相同作用域

`data_transform` 的 `name`（平面格式）或 `output`（嵌套格式）创建的变量，与 `aiQuery` 的 `name` 遵循相同的作用域规则。
  ...
  - name: 处理数据
    flow:
      - data_transform:
          ...
          name: "activeItems"          # 后续步骤和 task 均可访问 ${activeItems}

      - data_transform:
          input: "${activeItems}"
          operations:
            - sort: "name asc"
          output: "sortedItems"        # 嵌套格式的输出，同样全局可访问
```
...
| 声明方式 | 作用域 | 何时可用 |
|---------|--------|---------|
| `variables` 顶层声明 | 全局（所有 task） | 转译时静态替换 |
| `name` 字段（aiQuery 等） | 当前及后续 task | 运行时动态赋值 |
| `data_transform` name/output | 当前及后续 task | 运行时动态赋值 |
| 环境变量 `${ENV:NAME}` | 全局 | 运行时从系统环境读取 |
| `loop` 的 `itemVar`/`indexVar` | 仅循环体内 | 每次迭代更新 |

### `try` / `catch` / `finally` 缩进结构

`try`/`catch`/`finally` 的缩进规则容易出错。以下可视化展示正确的层级关系：

```
step 层级（flow 数组的直接子元素）
│
├─ try:                    ← step 的 key
│   flow:                  ← try 的子 key
│     - aiTap: "按钮"      ← try 的执行步骤
│     - aiWaitFor: "完成"
│
├─ catch:                  ← 与 try 同级（同一个 step 的 sibling key）
│   flow:                  ← catch 的子 key
│     - aiTap: "取消"      ← catch 的执行步骤
│
└─ finally:                ← 与 try/catch 同级
    flow:                  ← finally 的子 key
      - aiAssert: "状态正常"
```
...
flow:
  - try:                           # ← 缩进 4（flow 数组元素）
      flow:                        # ← 缩进 6（try 的子属性）
        - aiTap: "导出按钮"         # ← 缩进 8（flow 数组元素）
        - aiWaitFor: "导出完成"

    catch:                         # ← 缩进 4（与 try 同级，同属一个 step）
      error: err
      flow:
        - aiTap: "关闭对话框"

    finally:                       # ← 缩进 4（与 try/catch 同级）
      flow:
        - aiQuery:
            query: "是否有错误信息"
            name: "errorInfo"
...
```yaml
# 错误: catch 作为独立 step，不会关联到 try
flow:
  - try:
      flow:
        - aiTap: "按钮"
  - catch:                   # ← 错误！这是新的 step，不是 try 的 sibling
      flow:
        - aiTap: "取消"
```

> **核心规则**: `try`、`catch`、`finally` 必须在同一个 YAML mapping（同一个 `-` 数组元素）内，作为 sibling keys 存在，而不是各自独立的数组元素。
...
| `catch` 未关联到 `try` | `catch` 写成了独立的数组元素 | 确保 `try`/`catch`/`finally` 在同一个 `-` 元素内 |
| 循环内累积数据丢失 | 未用 `data_transform` 追加到数组 | 每次迭代用 `reduce` + `concat` 追加到累积变量 |
| `aiActContext` 未生效 | 写在了 task 内而非 `agent` 块 | 将 `aiActContext` 放在顶层 `agent` 配置中 |
...
在持续集成环境中运行 Midscene YAML 测试，可实现自动化回归验证。本章涵盖 GitHub Actions 配置、Docker 环境、安全管理和报告自动化。

### GitHub Actions 工作流

最小可用的 GitHub Actions 配置（Ubuntu + Chrome headless）：
  ...
  e2e:
    ...
    steps:
      ...
      - name: Run tests
        ...
        run: node scripts/midscene-run.js tests/*.yaml --timeout 60000
...
API 密钥等敏感信息应通过 CI 平台的 Secrets 机制注入，绝不硬编码到 YAML 文件中：

- 在 GitHub → Settings → Secrets 中添加 `MIDSCENE_MODEL_API_KEY`
- 在 workflow 的 `env:` 块中引用 `${{ secrets.MIDSCENE_MODEL_API_KEY }}`
- YAML 文件中使用 `${ENV:MIDSCENE_MODEL_API_KEY}` 引用环境变量
- 同理适用于 `MIDSCENE_MODEL_BASE_URL`、`MIDSCENE_MODEL_NAME` 等配置

### 报告产物管理

使用 `upload-artifact` 将 `midscene-report/` 目录上传为 CI 产物：

- 设置 `if: always()` 确保失败时也能查看报告和截图
- `retention-days` 建议 7-30 天，平衡存储成本与排查需要
- 报告包含 HTML 可视化文件和 AI 查询结果，是排查失败的首要入口

### Headless 模式配置

CI 环境无显示器，必须配置 headless 模式：
...
在 Docker 容器中运行时，需安装 Chrome 及相关依赖：
...
- `--no-sandbox` 在容器中通常是必须的（Chrome 以 root 运行时），但会降低安全隔离
- 安装 `fonts-noto-cjk` 或 `fonts-wqy-zenhei` 以支持中日韩文字正确渲染
- 生产环境建议使用非 root 用户运行 Chrome 以规避 `--no-sandbox`

### `.env` 安全管理

- **永远不要**将 `.env` 文件提交到版本控制（`.gitignore` 已默认排除）
- 本地开发使用 `.env` 文件，CI 环境通过 Secrets 注入
- 若需在 CI 中构建 `.env`，使用 `echo` 写入而非上传文件：

```yaml
- name: Create .env
  run: echo "MIDSCENE_MODEL_API_KEY=${{ secrets.MIDSCENE_MODEL_API_KEY }}" > .env
```

### CI 后清理

每次 CI 运行后清理临时文件，避免残留影响后续执行：

```yaml
- name: Clean temp files
  if: always()
  run: node scripts/midscene-run.js --clean
```

`--clean` 会删除 `midscene_run/` 下超过阈值时间的临时文件，保持 CI 环境整洁。

### 批量执行与重试策略

CI 中通常需要批量运行多个 YAML 文件，并对不稳定测试进行重试：

```bash
# glob 模式批量执行
node scripts/midscene-run.js "tests/**/*.yaml" --timeout 60000

# 不稳定测试自动重试 2 次
node scripts/midscene-run.js "tests/**/*.yaml" --retry 2
...
- glob 模式支持 `*`、`**`、`?` 等通配符
- `--retry 2` 在测试失败时最多重试 2 次（每次间隔 2 秒）
- `--max-files 50` 防止匹配过多文件导致 CI 超时

### 报告自动化

利用 CLI 退出码和报告解析实现 CI 自动化判断：
...
- name: Run tests and check results
  run: |
    ...
    if [ $EXIT_CODE -ne 0 ]; then
      echo "::error::Midscene tests failed with exit code $EXIT_CODE"
      exit $EXIT_CODE
    fi
```

- CLI 成功时退出码为 `0`，失败时为非零值
- 结合 `upload-artifact` 上传报告目录，失败时可通过 HTML 报告查看截图和错误详情
- 报告解析器 `report-parser.js` 提供 `parse()` 方法，可编写自定义脚本提取 `failedTasks`、`totalDuration` 等信息
...
| `ai` / `aiAct` / `aiAction` | 动作 | AI 自动规划并执行自然语言描述的操作（推荐使用 `aiAct`，`aiAction` 亦为有效别名） |
...
> 完整的 CI/CD 集成指南请参阅主章节 [CI/CD 集成](#cicd-集成)。以下为快速参考摘要。
...
| **超时** | CI 网络可能较慢，建议 `--timeout 60000` 以上 |
| **报告产物** | `upload-artifact` + `if: always()` + `retention-days` |
| **Chrome** | Ubuntu runner 自带；Docker 中需手动安装 + `--no-sandbox` |
| **重试** | `--retry 2` 处理不稳定测试 |
| **清理** | CI 后运行 `--clean` 清除临时文件 |
...
**A**: Extended 模式默认使用 Playwright 模板（`--template playwright`），支持 Chromium、Firefox、WebKit。如需使用 Puppeteer，可指定 `--template puppeteer`。Native 模式目前仅支持 Chromium/Chrome。
...
## 附录 H: VS Code YAML Schema 集成

项目提供了完整的 JSON Schema（`schema/yaml-superset-schema.json`），可在 VS Code 中实现 YAML 自动补全、语法校验和悬停提示。

### 配置步骤

1. 安装 VS Code 扩展 [YAML](https://marketplace.visualstudio.com/items?itemName=redhat.vscode-yaml)（Red Hat 出品）。
2. 在项目根目录创建或编辑 `.vscode/settings.json`，添加以下配置：

```json
{
  "yaml.schemas": {
    "./schema/yaml-superset-schema.json": "*.yaml"
  }
}
```

3. 重新打开任意 `.yaml` 文件，即可获得：
   - **自动补全**: 输入顶层字段（`web`、`tasks`、`agent` 等）时自动提示
   - **校验提示**: 拼写错误或不支持的字段会标红
   - **悬停文档**: 鼠标悬停在字段名上显示 description 说明

### 作用域限制

如果只希望对特定目录下的 YAML 文件启用 Schema，可使用 glob 模式：

```json
{
  "yaml.schemas": {
    "./schema/yaml-superset-schema.json": ["midscene-output/**/*.yaml", "templates/**/*.yaml"]
  }
}
```

> **提示**: Schema 文件路径是相对于项目根目录的。如果使用全局安装，需替换为绝对路径。

---

## 附录 I: 多语言配置 (MIDSCENE_PREFERRED_LANGUAGE)

Midscene 支持通过环境变量 `MIDSCENE_PREFERRED_LANGUAGE` 设置 AI 指令的语言偏好，影响 AI 对自然语言描述的解读和响应语言。
...
```bash
MIDSCENE_PREFERRED_LANGUAGE=en
```
...
```bash
# Linux / macOS
MIDSCENE_PREFERRED_LANGUAGE=ja node scripts/midscene-run.js test.yaml

# Windows (PowerShell)
$env:MIDSCENE_PREFERRED_LANGUAGE="ja"; node scripts/midscene-run.js test.yaml
```

**方式三**: 在 CI 环境中配置（以 GitHub Actions 为例）：
...
- AI 动作（`aiTap`、`aiAssert`、`aiQuery` 等）的自然语言解读
- AI 自动规划（`ai` / `aiAct`）的指令理解
- 报告中 AI 相关的输出文本

> **注意**: 该设置影响的是 AI 模型对指令的理解偏好，YAML 中的自然语言描述仍可使用任意语言。设置为 `en` 后，即使描述用中文写，AI 也会优先以英文思维解析。

---

## 附录 J: Bridge Mode 桥接模式

桥接模式（Bridge Mode）允许 Midscene 连接到一个已运行的浏览器实例，而不是自动启动新浏览器。适用于调试、复用登录状态、与其他工具共享浏览器等场景。

### 工作原理

普通模式下，Midscene 会自动启动一个新的浏览器进程并在结束后关闭。桥接模式则跳过启动步骤，直接连接到已有的浏览器会话。

### 配置方式

在 `web` 块中设置 `bridgeMode` 字段：

```yaml
web:
  url: "https://example.com"
  bridgeMode: "newTabWithUrl"    # 在已有浏览器中打开新标签页
```
...
| 值 | 行为 |
|---|---|
| `false` | 禁用桥接模式（默认值），自动启动新浏览器 |
| `"newTabWithUrl"` | 在已有浏览器中打开新标签页并导航到 `url` |
| `"currentTab"` | 直接使用已有浏览器的当前标签页 |

### 使用场景

**场景一: 调试模式** — 手动打开浏览器并登录后，用 Midscene 执行后续操作：
...
**场景二: 复用登录状态** — 浏览器已完成 OAuth 登录，YAML 直接在新标签页操作：
...
> **提示**: 桥接模式下，`headless` 设置无效（浏览器已由用户手动启动）。`chromeArgs` 同样不生效。

---

## 附录 K: Agent 配置完整参考

`agent` 块用于配置 Midscene Agent 的行为参数。所有字段均为可选。
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `testId` | string | 测试唯一标识符，用于区分不同测试运行 |
| `groupName` | string | 测试分组名称，报告中按组归类 |
| `groupDescription` | string | 测试分组描述，报告中显示 |
| `cache` | boolean \| object | 缓存策略。`true` 启用默认缓存；对象格式支持 `strategy` 和 `id` |
| `generateReport` | boolean | 是否生成 HTML 报告（默认 true） |
| `autoPrintReportMsg` | boolean | 执行完毕后是否自动打印报告路径 |
| `reportFileName` | string | 自定义报告文件名 |
| `replanningCycleLimit` | integer | AI 重新规划的最大循环次数（默认 20，UI-TARS 模型为 40） |
| `aiActContext` | string | AI 动作的背景知识，会注入到所有 AI 指令的上下文中 |
| `screenshotShrinkFactor` | number | 截图缩小倍数（最小 1）。值越大分辨率越低，节省 token 消耗。例如 2 表示缩小为原来的一半 |
| `waitAfterAction` | integer | 每个动作执行后的等待时间（毫秒，默认 300） |
| `outputFormat` | string | 报告输出格式：`"single-html"` 或 `"html-and-external-assets"` |
| `modelConfig` | object | 覆盖全局模型配置，支持任意键值对 |
...
agent:
  cache:
    strategy: "read-write"    # "read-write" | "read-only" | "write-only"
    id: "my-cache-id"         # 缓存标识符（建议设置，strategy 有值时如不设 id 会产生警告）
```

### aiActContext 用法

为所有 AI 动作提供全局背景知识，帮助 AI 更准确地理解页面：

```yaml
agent:
  aiActContext: "这是一个电商后台管理系统，使用 Ant Design 组件库。表格数据为商品信息。"
```

### modelConfig 用法

覆盖单个 YAML 文件的模型配置，优先级高于环境变量：
...
以下是 Midscene 支持的所有环境变量，可在 `.env` 文件或系统环境中配置。

### 模型配置（必需）

| 环境变量 | 说明 |
|---|---|
| `MIDSCENE_MODEL_API_KEY` | AI 模型的 API Key（必需） |
| `MIDSCENE_MODEL_NAME` | 模型名称，如 `qwen-vl-max-latest`、`gpt-4o`、`gemini-2.0-flash` |
| `MIDSCENE_MODEL_BASE_URL` | API 端点地址（即 Endpoint），如 `https://api.openai.com/v1` |

### 语言与运行时

| 环境变量 | 说明 |
|---|---|
| `MIDSCENE_PREFERRED_LANGUAGE` | AI 响应语言偏好：`en`、`zh`、`ja`。GMT+8 时区默认 `zh`，其他默认 `en` |
| `MIDSCENE_RUN_DIR` | 运行时产物目录（报告、缓存），默认 `./midscene_run` |
| `MIDSCENE_MODEL_FAMILY` | 模型家族标识，用于适配不同模型的特性 |

### 高级模型配置

| 环境变量 | 说明 |
|---|---|
| `MIDSCENE_INSIGHT_MODEL_*` | 为 Insight（元素定位）阶段单独配置模型（`*` 替换为 `API_KEY`、`NAME`、`BASE_URL`） |
| `MIDSCENE_PLANNING_MODEL_*` | 为 Planning（操作规划）阶段单独配置模型 |
...
| 环境变量 | 说明 |
|---|---|
| `MIDSCENE_MODEL_HTTP_PROXY` | AI API 的 HTTP 代理地址 |
| `MIDSCENE_MODEL_SOCKS_PROXY` | AI API 的 SOCKS 代理地址 |

### 调试

| 环境变量 | 说明 |
|---|---|
| `DEBUG` | 启用调试日志。常用命名空间：`midscene:ai:profile:detail`（token 详情）、`midscene:android:adb`（ADB 调试） |
...
MIDSCENE_PREFERRED_LANGUAGE=zh
MIDSCENE_RUN_DIR=./midscene_run
```

```bash
# .env 文件示例 — 使用 OpenAI GPT-4o
...
> **安全提示**: `MIDSCENE_MODEL_API_KEY` 等密钥不要硬编码在 YAML 文件中或提交到版本库。在 CI 环境中应通过 Secrets 管理。
