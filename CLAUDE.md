...
- `planningStrategy` 可选字符串属性：ai/aiAct/aiAction 支持
...
- `resolveTemplate` 模板表达式安全校验：仅允许简单标识符路径，拒绝 `()`、`require`、`eval`、`import`、`Function`、`__proto__`、`constructor`、`prototype`
...
| `ssl_certificate` | recoverable | SSL/TLS 证书错误 |
| `disk_full` | fatal | 磁盘空间不足 |
| `memory_exhaustion` | fatal | 内存不足（OOM） |

CLI 退出码：0=成功、1=执行失败、2=验证/配置错误
